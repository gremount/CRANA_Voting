...
public:
	...
	void evaluate(VGraph &g, vector<Player*> &candiL)
	{
		...
		for(int i=0;i<candiL.size();i++)
		{
			...
			edge_num=candiL[i]->path_record[id]->pathL.size();
			temp=0;
			for(int j=0;j<edge_num;j++)
			{
				int src=candiL[i]->path_record[id]->pathL[j]->src;
				int dst=candiL[i]->path_record[id]->pathL[j]->dst;
...
double judge_sum_function(VGraph &g, vector<Player*> &candiL, int winner)
{
	...
	for(int i=0;i<g.incL.size();i++)
	{
		...
		if(candiL[winner]->adj[src][dst]==0) continue;//由于延时函数是1+x2/(c-x1)，所以即使没有流量，
		//依然会在统计中算入延时，所以这里判断一下来消除这些误判
		latencyTemp = (double)candiL[winner]->adj[src][dst]/(1+ g.incL[i]->capacity - candiL[winner]->adj[src][dst]);
